...
def apply_template(template_path, image_path, coords):
    if template_path is None or image_path is None:
        return None
...
def main():
    ...
    def update_preview():
        ...
        if aplicare_logo.get() and logo is not None:
            h, w, _ = result.shape if result is not None else (img_tk.height(), img_tk.width(), 3)
            if template_type.get() == "fata":
                x_offset = w - logo.shape[1] - (325 if not extinde.get() else 310) #daca scad valoarea lui else logo merge in dreapta
                y_offset = h - logo.shape[0] - 5  # Aici modifica valoarea pentru y
            else:
                x_offset = 325 if not extinde.get() else 312  # Aici modifica valoarea pentru x
                y_offset = h - logo.shape[0] - 5  # Aici modifica valoarea pentru y
            result = apply_logo(result if result is not None else cv2.imread(template_path), logo, (x_offset, y_offset))
    ...
    def refresh_app():
        root.destroy()  # Închide fereastra curentă
        python = sys.executable
        subprocess.Popen([python, *sys.argv])  # Repornire fără probleme de spații
        sys.exit()  # Închide complet procesul curent
    ...
    tk.Button(button_frame, text="Refresh", command=refresh_app).pack(pady=5)
